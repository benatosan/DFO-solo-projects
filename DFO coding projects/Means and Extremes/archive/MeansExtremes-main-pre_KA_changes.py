...
class dmf:
    ...
    def make_file(self) -> None:
        ...
        with open('CORNWALL_DMF_2024.dat', 'w') as f:
            ...
            for stationCode in header_dict:
                #For every station, go through every month
                for month in months:
                    #Header
                    f.write(f"- {stationCode} {year}{month:02d}{header_dict[str(stationCode)]}\n")
                    ...
                    for day in range(1,days_in_month[month-1]+1, 10):     # num of days is dictated by the days_in_month list, we get index from month-1
                        if day == 1:
                            ...
                            dailyMeans = self.get_dailyMeans(api.get_station_id(station_code=stationCode), month, range(1, 11))
                            #Write the line into the file
                            f.write(f"5 {stationCode} {year}{month:02d}0110      {dailyMeans}\n")
                        elif day == 11:
                            #Get the daily mean for current 10-day period and station
                            dailyMeans = self.get_dailyMeans(api.get_station_id(station_code=stationCode), month, range(11, 21))
                            #Write the line into the file
                            f.write(f"5 {stationCode} {year}{month:02d}1120      {dailyMeans}\n")
                        else:
                            #Get the daily mean for current 10-day period and station
                            dailyMeans = self.get_dailyMeans(api.get_station_id(station_code=stationCode), month, range(21, days_in_month[month-1]+1))
                            #Write the line into the file
                            f.write(f"5 {stationCode} {year}{month:02d}21{days_in_month[month-1]}{self.get_monthly_mean()}{dailyMeans}\n")
                            #Write the line with code 6 to the file
                            f.write(f"6 {stationCode} {year}{month:02d}     9999999999999999999999 999 99\n")
                            break
        return
                        
    #------------------------------------------------------------------------------------------------------------------------------------------

    def get_dailyMeans(self, stationId: object, month: int, days: list) -> str:
        '''
        ...
        for day in days:
            ...
            try:
                ...
                response = requests.get(f"https://api-iwls.dfo-mpo.gc.ca/api/v1/stations/{params['stationId']}/stats/calculate-daily-means-igld85", params=params)

                if response.status_code == 200:
                    #load data as json
                    data = response.json()
                    print(data)
                else: print(f"Bad response, error code: {response.status_code}"); return
            ...
            try:
                ...
                self.dailyMeansForMonth.append(data[0]['dailyMean_IGLD85'])
                #Concat to string that will be put in the file    
                finalString += "{:.2f}".format(data[0]['dailyMean_IGLD85']).replace('.', '')
